...
export function WorkersClient({ initialWorkers, initialMeta, pageSize }: WorkersClientProps) {
  const { t } = useTranslations();
  const [workersState, setWorkersState] = useState<Worker[]>(initialWorkers);
  const [meta, setMeta] = useState<WorkerListMeta>(initialMeta);
  ...
  const fetchWorkersPage = async ({ offset, append }: { offset: number; append: boolean }) => {
    if (append) {
    ...
    try {
      const response = await fetch(
        `/api/workers?limit=${pageSize}&offset=${offset}&search=${encodeURIComponent(debouncedSearch)}`
      );
      const json = await response.json();
      if (!response.ok) {
        throw new Error(json.error || "Failed to load workers");
      }
      const incoming: Worker[] = json.data ?? [];
      setWorkersState((prev) => (append ? [...prev, ...incoming] : incoming));
  ...
  useEffect(() => {
    ...
    fetchWorkersPage({ offset: 0, append: false });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedWorker) {
        ...
        setMeta((prev) => {
          const total = (prev?.total ?? prev.limit ?? workersState.length) + 1;
          const countAfterInsert = workersState.length + 1;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setMeta((prev) => {
        const total = Math.max(0, (prev?.total ?? prev.limit ?? workersState.length) - 1);
        const remaining = Math.max(0, workersState.length - 1);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <WorkerList
        ...
        onLoadMore={() => fetchWorkersPage({ offset: workersState.length, append: true })}
      />
