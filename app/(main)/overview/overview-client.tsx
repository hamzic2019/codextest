...
type AvailablePlan = { month: number; year: number };

type PlanOverviewClientProps = {
  ...
  initialAvailablePlans: AvailablePlan[];
};

export function PlanOverviewClient({
  initialPatients,
  initialWorkers,
  initialPatientId,
  initialYear,
  initialMonth,
  initialAssignments,
  initialAvailablePlans,
}: PlanOverviewClientProps) {
  const { t, language } = useTranslations();
  const monthNames = MONTH_LABELS[language] ?? MONTH_LABELS.bs;
  const todayDate = useMemo(() => new Date(), []);
  const todayDisplay = `${todayDate.getDate()}. ${monthNames[todayDate.getMonth()]}`;

  const patientsState = initialPatients;
  const workersState = initialWorkers;
  const [planAssignments, setPlanAssignments] = useState<PlanAssignment[]>(initialAssignments);
  const [selectedPatient, setSelectedPatient] = useState<string>(initialPatientId || "");
  const [selectedYear, setSelectedYear] = useState(initialYear);
  const [selectedMonth, setSelectedMonth] = useState(initialMonth);
  const [availablePlans, setAvailablePlans] = useState<AvailablePlan[]>(initialAvailablePlans);
  ...
  const availableYears = useMemo(() => {
    ...
    return unique.length > 0 ? unique : [selectedYear];
  }, [availablePlans, selectedYear]);
  ...
  useEffect(() => {
    ...
    if (!hasHydratedInitial.current) {
      hasHydratedInitial.current = true;
      return;
    }
    const loadAvailablePlans = async () => {
      try {
        const response = await fetch(`/api/plans?patientId=${selectedPatient}&mode=available`);
  ...
  }, [initialMonth, initialPatientId, initialYear, selectedMonth, selectedPatient, selectedYear, t]);

  useEffect(() => {
    if (!selectedPatient && patientsState.length > 0) {
      setSelectedPatient(patientsState[0].id);
    }
  }, [patientsState, selectedPatient]);
